...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if lq.withCompany != nil || lq.withCountry != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withCompany; query != nil {
		if err := lq.loadCompany(ctx, query, nodes, nil,
			func(n *Location, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
...
func (lq *LocationQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Location, init func(*Location), assign func(*Location, *Company)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
		if nodes[i].company_locations == nil {
			continue
		}
		fk := *nodes[i].company_locations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_locations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (lq *LocationQuery) loadCountry(ctx context.Context, query *CountryQuery, nodes []*Location, init func(*Location), assign func(*Location, *Country)) error {
	...
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
		if nodes[i].country_locations == nil {
			continue
		}
		fk := *nodes[i].country_locations
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "country_locations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
