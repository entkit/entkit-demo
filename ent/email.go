...
type Email struct {
	...
	company_emails *uuid.UUID
	country_emails *uuid.UUID
...
func (*Email) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case email.FieldTitle, email.FieldDescription, email.FieldAddress:
			values[i] = new(sql.NullString)
		case email.FieldID:
			values[i] = new(uuid.UUID)
		case email.ForeignKeys[0]: // company_emails
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		case email.ForeignKeys[1]: // country_emails
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		default:
	...
	for i := range columns {
		...
		case email.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field company_emails", values[i])
			} else if value.Valid {
				e.company_emails = new(uuid.UUID)
				*e.company_emails = *value.S.(*uuid.UUID)
			}
		case email.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field country_emails", values[i])
			} else if value.Valid {
				e.country_emails = new(uuid.UUID)
				*e.country_emails = *value.S.(*uuid.UUID)
			}
		default:
